...
function getEtiquetasDoDia() {
  ...
  // Parts map — key: produto → { parte: N, qtdNaParte: X }
  var partesMap = {};
  ...
  if (partesSheet && partesSheet.getLastRow() >= 2) {
    ...
    partesData.forEach(function(r) {
      if (r[0]) partesMap[String(r[0])] = { parte: Number(r[1]) || 0, qtdNaParte: Number(r[2]) || 0 };
    });
  }
  ...
  pendentes.forEach(function(item) {
    var parteInfo = partesMap[item.produto];
    var parteNum  = parteInfo ? parteInfo.parte : 0;
    var qtdNaP    = parteInfo ? parteInfo.qtdNaParte : item.qtdNecessaria;

    var qtdItem = parteNum > 0 ? qtdNaP : item.qtdNecessaria;
    var chave   = item.produto + '|' + parteNum;
    var estado  = estadoMap[chave] || { qtdEncontrada: 0, observacao: '' };

    result.push({
      produto:       item.produto,
      qtdNecessaria: qtdItem,
      qtdEncontrada: Math.min(estado.qtdEncontrada, qtdItem),
      encontrado:    estado.qtdEncontrada >= qtdItem,
    ...
    // Extra "Novos" item when qty increased after division
    if (parteNum > 0 && item.qtdNecessaria > qtdNaP) {
      var qtdExtra = item.qtdNecessaria - qtdNaP;
