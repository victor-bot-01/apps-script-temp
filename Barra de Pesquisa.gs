...
// Reads from Pedidos_Producao (Para Produção page source)
...
function getEtiquetasDoDia() {
  ...
  // Build set of products in Pedidos_Producao
  var pedidosSet = {};
  ...
  // Build part map from Etiquetas_Partes
  var partesMap = {};
  var partesSheet = ss.getSheetByName('Etiquetas_Partes');
  if (partesSheet && partesSheet.getLastRow() >= 2) {
    var partesData = partesSheet.getRange(2, 1, partesSheet.getLastRow() - 1, 2).getValues();
    partesData.forEach(function(r) {
      if (r[0]) partesMap[String(r[0])] = Number(r[1]) || 0;
    });
  }
  ...
  return pendentes.map(function(item) {
    ...
    return {
      ...
      parte:         partesMap[item.produto] || 0
    };
  });
}
...
function criarParteEtiqueta(produtos) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sh = ss.getSheetByName('Etiquetas_Partes');
  if (!sh) {
    sh = ss.insertSheet('Etiquetas_Partes');
    sh.getRange(1, 1, 1, 2).setValues([['Produto', 'Parte']]);
    sh.getRange(1, 1, 1, 2).setFontWeight('bold');
  }

  // Find current max part number
  var maxParte = 0;
  if (sh.getLastRow() >= 2) {
  ...
  // Save all produtos with this part number
  var rows = produtos.map(function(p) { return [p, numParte]; });
  if (rows.length) {
