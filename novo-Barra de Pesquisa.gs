  ...
  Object.keys(msgProdutosMap).forEach(function(msgId) {
    ...
    confRows.forEach(function(conf) {
      ...
      if (candidates.length > 1) {
        // Tenta casar col D da Conferência com um único candidato via keywords (caso: múltiplas linhas, 1 produto por linha).
        // Se não houver match único, expande todos os candidatos (caso: 1 linha = Kit com N produtos individuais).
        var confNomeLower = (conf.produtoConf || '').toLowerCase();
        var confMatch = candidates.filter(function(p) {
          var kws = p.toLowerCase().split(/\s+/).filter(function(w) { return w.length > 2; });
          return kws.length > 0 && kws.every(function(kw) { return confNomeLower.indexOf(kw) !== -1; });
        });
        if (confMatch.length === 1) {
          newPedidos.push({
