...
function getEtiquetasDoDia() {
  ...
  // Lê Lista col D + F (fonte principal — inclui FALTA, TENHO e em branco)
  ...
  if (confSheet && confSheet.getLastRow() >= 2) {
    ...
    confData.forEach(function(r, idx) {
      ...
      confMap[msgId].push({
        ...
        confirmado:  (numCols >= 12) ? String(r[11] || '').trim().toUpperCase() : ''
      });
    });
  }

  // Dados: nome do anúncio (Conferência col D) → [sub-itens separados por ;]
  var dadosMap = {};
  ...
  if (dadosSheet && dadosSheet.getLastRow() >= 1) {
    dadosSheet.getDataRange().getValues().forEach(function(row) {
      for (var c = 0; c < row.length - 1; c++) {
        ...
        var subRaw = String(row[c + 1] || '').trim();
        if (nomeAd && subRaw) {
          dadosMap[nomeAd] = subRaw.split(';').map(function(s) { return s.trim(); }).filter(Boolean);
          c++;
        }
      }
    });
  ...
  // Descobre quais msgIds estão na Lista (inclui TENHO, FALTA e em branco)
  var msgIdsNaLista = {};
  ...
  // Nomes que são automaticamente "Encontrado" (match exato, sem distinção de acentos/maiúsculas)
  var AUTO_ENCONTRADO_SET = (function() {
    var names = [
      'pingente','pingentes','colar','colares','anel','anéis','aneis',
      'pulseira','pulseiras','corrente','correntes','brinco','brincos',
      'berloque','berloques','gema','gemas','larimar','larimares',
      'carteira','carteiras','nécessaire','necessaire','nécessaires','necessaires',
      'sérum','serum','séruns','seruns','hidratante','hidratantes','bolsa','bolsas'
    ];
    var obj = {};
    names.forEach(function(n) {
      obj[n.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '')] = true;
    });
    return obj;
  })();
  function isAutoEncontrado(nome) {
    var words = String(nome).trim().toLowerCase()
      .normalize('NFD').replace(/[̀-ͯ]/g, '')
      .split(/\s+/);
    return words.some(function(w) { return !!AUTO_ENCONTRADO_SET[w]; });
  }

  // Normaliza texto para keyword matching
  function normKw(s) {
  ...
  // Para combos com "FALTA - xxx": retorna o índice do sub-item que mais bate com o detalhe
  function findFaltaIdx(subItens, faltaDetail) {
    var kD = normKw(faltaDetail);
    var scores = subItens.map(function(si) {
      var kS = normKw(si), n = 0;
      kS.forEach(function(w) { if (kD.indexOf(w) !== -1) n++; });
      return n;
    });
  ...
  Object.keys(confMap).forEach(function(msgId) {
    if (!msgIdsNaLista[msgId]) return;
    confMap[msgId].forEach(function(conf) {
      if (!conf.marketplace || !conf.lineKey) return;

      var s        = String(conf.status || '').trim().toUpperCase();
      var subItens = dadosMap[conf.produtoConf];
      if (!subItens || !subItens.length) return; // produto não cadastrado no Dados → ignora

      // 1 sub-item = produto simples; 2+ sub-itens separados por ; = combo
      var isTenho     = (s === 'TENHO');
      var faltaDetail = (s.indexOf('FALTA - ') === 0) ? s.substring(8).trim() : '';
      var faltaIdx    = (faltaDetail && subItens.length > 1) ? findFaltaIdx(subItens, faltaDetail) : -1;

      subItens.forEach(function(subItem, idx) {
        var forceEnc = (conf.confirmado === 'SIM') || isAutoEncontrado(subItem);
        var initialStatus;
        if (forceEnc) {
          ...
          initialStatus = (idx === faltaIdx) ? '' : 'Encontrado';
        } else {
          initialStatus = '';
        }
        newPedidos.push({
  ...
  // Mapa de discordIds que devem ser forçados como "Encontrado"
  var forceEncMap = {};
  newPedidos.forEach(function(p) { if (p.forceEncontrado) forceEncMap[p.discordId] = true; });
  ...
  // Força "Encontrado" em entradas já existentes que cumprem as regras de auto-confirmação
  Object.keys(forceEncMap).forEach(function(did) {
    if (estadoMap[did] && estadoMap[did].status !== 'Encontrado') {
      estadoSh.getRange(estadoMap[did].row, 6).setValue('Encontrado');
      estadoMap[did].status = 'Encontrado';
    }
  });
