...
function getEtiquetasDoDia() {
  ...
  // Mapeamento msgId → [produtos FALTA]: Lista col D + col F
  // Usa keyword-subset matching para lidar com variações de nome entre col A e col D
  // (ex: col A = "Kit 3 Perfumes Roll On 10ml Feminino", col D = "... - Oriental Marcante")
  var listaDF = listaSheet.getRange(2, 4, listaSheet.getLastRow() - 1, 3).getValues();

  var pendentesKwMap = {};
  pendentes.forEach(function(p) {
    pendentesKwMap[p.produto] = p.produto.toLowerCase()
  ...
  listaDF.forEach(function(r) {
    ...
    if (!produtoD || !idsRaw) return;

    // 1) Correspondência exata
    var matchedPendente = pendentesSet[produtoD] ? produtoD : null;

    // 2) Fallback: todos os keywords do pendente (col A) devem estar no nome col D
    if (!matchedPendente) {
      var nomeD = produtoD.toLowerCase();
      var bestScore = 0;
      pendentes.forEach(function(p) {
        var kwsA = pendentesKwMap[p.produto];
        if (!kwsA || !kwsA.length) return;
        var hits = kwsA.filter(function(kw) { return nomeD.indexOf(kw) !== -1; }).length;
        if (hits === kwsA.length && hits > bestScore) {
          bestScore = hits;
          matchedPendente = p.produto;
        }
      });
    }

    if (!matchedPendente) return;

    idsRaw.split(/\s+/).forEach(function(id) {
      if (!id) return;
      if (!msgProdutosMap[id]) msgProdutosMap[id] = [];
      if (msgProdutosMap[id].indexOf(matchedPendente) === -1) msgProdutosMap[id].push(matchedPendente);
    });
  });
