...
function getEtiquetasDoDia() {
  ...
  // Pedidos novos: TENHO — entram como "Encontrado" em Etiquetas_Estado
  // Itera confMap (não msgProdutosMap) para alcançar pedidos exclusivamente TENHO
  var matchedLineKeysTenho = {};
  newPedidos.forEach(function(p) { matchedLineKeysTenho[p.discordId] = true; });

  Object.keys(confMap).forEach(function(msgId) {
    var confRows = confMap[msgId];
    var temTenho = confRows.some(function(c) { return String(c.status || '').trim().toUpperCase() === 'TENHO'; });
    if (!temTenho) return;

    // poolT é compartilhado entre todas as linhas TENHO do mesmo pedido (evita reutilizar o mesmo produto)
    var poolT = (msgProdutosMap[msgId] || []).slice();

    confRows.forEach(function(conf) {
      if (!conf.marketplace) return;
      if (String(conf.status || '').trim().toUpperCase() !== 'TENHO') return;

      // 1ª opção: dadosMap (combos/kits — uma linha → vários produtos da Lista)
      var dadosProds = dadosMap[conf.produtoConf];
      if (dadosProds && dadosProds.length) {
        var pushed = false;
        dadosProds.forEach(function(p, idx) {
          if (!pendentesSet[p]) return;
          var did = conf.lineKey + (idx > 0 ? '_' + idx : '');
          if (matchedLineKeysTenho[did]) return;
          newPedidos.push({
            discordId:     did,
            produto:       p,
            idPedido:      conf.idPedido,
            marketplace:   conf.marketplace,
            qtd:           conf.qtd,
            initialStatus: 'Encontrado'
          });
          matchedLineKeysTenho[did] = true;
          pushed = true;
          var pi = poolT.indexOf(p);
          if (pi !== -1) poolT.splice(pi, 1);
        });
        if (pushed) return; // só sai se de fato adicionou algo
      }

      // 2ª opção (fallback): score por keywords do col D usando poolT
      var candidates = poolT.filter(function(p) { return pendentesSet[p]; });
      if (!candidates.length) return;
