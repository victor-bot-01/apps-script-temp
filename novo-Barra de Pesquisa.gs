...
function getEtiquetasDoDia() {
  ...
  // Constrói newPedidos diretamente: Lista col D → col F (Discord IDs) → confMap (lineKey)
  var newPedidos = [];
  var seenLineKeys = {};

  listaDEF.forEach(function(r) {
    var produtoD = String(r[0] || '').trim(); // col D = nome do produto (mesmo que Conferência)
    var idsRaw   = String(r[2] || '').trim(); // col F = Discord IDs separados por espaço
    ...
    idsRaw.split(/\s+/).forEach(function(msgId) {
      ...
      confRows.forEach(function(conf) {
        ...
        if (seenLineKeys[conf.lineKey]) return;
        seenLineKeys[conf.lineKey] = true;
