...
function getEtiquetasDoDia() {
  ...
  if (avulsoSh && avulsoSh.getLastRow() >= 2) {
    ...
    avulsoData.forEach(function(r) {
      ...
      // Aceita avulsos mesmo se o produto for FALTA — pode estar "Tenho" na Conferência
  ...
  // Adiciona avulsos a pendentes; se o produto já existe como FALTA, aumenta qtdNecessaria
  Object.keys(avulsosMap).forEach(function(prod) {
    if (!pendentesSet[prod]) {
      pendentes.push({ produto: prod, qtdNecessaria: avulsosMap[prod] });
      pendentesSet[prod] = true;
    } else {
      // Produto já está em pendentes (FALTA) — avulso define o total necessário do dia
      for (var i = 0; i < pendentes.length; i++) {
        if (pendentes[i].produto === prod) {
          if (avulsosMap[prod] > pendentes[i].qtdNecessaria) {
            pendentes[i].qtdNecessaria = avulsosMap[prod];
          }
          break;
        }
      }
  ...
  // dadosMap: nome do anúncio (col D da Conferência) → [produtos da Lista, semicolon-separated na Dados]
  var dadosMap = {};
  var dadosSheet = extSS.getSheetByName('Dados');
  if (dadosSheet && dadosSheet.getLastRow() >= 1) {
    var dadosData = dadosSheet.getDataRange().getValues();
    dadosData.forEach(function(row) {
      for (var c = 0; c < row.length - 1; c++) {
        ...
        if (nomeAd && desMemRaw) {
          ...
          c++; // célula seguinte já consumida como desmembramento
        }
      }
    });
  }
  ...
  function scoreConfMatch(produto, confNome) {
    var kws = confNome.toLowerCase().replace(/[^a-z0-9çáéíóúãõâêôàèìòù]/g, ' ').split(/\s+/).filter(function(w) { return w.length > 2; });
    if (!kws.length) return 0;
    var pL = produto.toLowerCase();
    return kws.filter(function(kw) { return pL.indexOf(kw) !== -1; }).length;
  }

  Object.keys(msgProdutosMap).forEach(function(msgId) {
    var pool = msgProdutosMap[msgId].slice();
    ...
    confRows.forEach(function(conf) {
      ...
      // 1ª opção: lookup direto na aba Dados (exato e confiável)
      var dadosProds = dadosMap[conf.produtoConf];
      if (dadosProds && dadosProds.length) {
        var matched = dadosProds.filter(function(p) { return produtoFaltaNoPedido(conf.status, p); });
        if (matched.length) {
      ...
      // 2ª opção (fallback): candidatos do pool + score por keywords do col D
      var candidates = pool.filter(function(p) { return produtoFaltaNoPedido(conf.status, p); });
      ...
      if (candidates.length === 1) {
        ...
        var scores = candidates.map(function(p) { return scoreConfMatch(p, conf.produtoConf || ''); });
        var maxScore = Math.max.apply(null, scores);
        picked = candidates[scores.indexOf(maxScore)];
      }
      newPedidos.push({
      ...
      var pi = pool.indexOf(picked);
      if (pi !== -1) pool.splice(pi, 1);
    });
  });

  // Pedidos novos: Avulsos — link direto por Discord ID (sem keyword matching)
  // Monta índice de produto+idPedido já criados pelo fluxo FALTA para evitar duplicatas
  var faltaPedidoProduto = {};
  newPedidos.forEach(function(p) { faltaPedidoProduto[p.produto + '|' + p.idPedido] = true; });
  ...
  Object.keys(avulsoMsgMap).forEach(function(msgId) {
    ...
    // Usa a primeira confRow do msgId como âncora do pedido
    var conf = confRows[0];
    if (!conf.marketplace) return;

    avulsoProds.forEach(function(prod, idx) {
      // Não cria se FALTA já criou entrada para esse produto+pedido
      if (faltaPedidoProduto[prod + '|' + conf.idPedido]) return;
      newPedidos.push({
        discordId:   conf.lineKey + '_av' + (idx > 0 ? idx : ''),
        produto:     prod,
        idPedido:    conf.idPedido,
        marketplace: conf.marketplace,
        qtd:         conf.qtd
  ...
  // Avulsos sem match via Conferência → entrada direta com ID gerado
  var matchedAvulsos = {};
  newPedidos.forEach(function(p) { if (avulsosMap[p.produto]) matchedAvulsos[p.produto] = true; });
  Object.keys(avulsosMap).forEach(function(prod) {
    if (matchedAvulsos[prod]) return;
  ...
  // Exceção: se estava "Não Encontrado" e a linha da Conferência ainda existe (mudou FALTA → Tenho),
  // o produto chegou sem etiqueta — mantém entrada e registra como avulso automaticamente
  var validIds = {};
  newPedidos.forEach(function(p) { validIds[p.discordId] = true; });

  // Índice lineKey → conf para detectar mudança de FALTA para Tenho
  var lineKeyToConf = {};
  Object.keys(confMap).forEach(function(msgId) {
    confMap[msgId].forEach(function(conf) {
      lineKeyToConf[conf.lineKey] = conf;
    });
  });

  // Produtos já em avulsosMap para não duplicar no Etiquetas_Avulsos
  var existingAvulsoProds = {};
  Object.keys(avulsosMap).forEach(function(p) { existingAvulsoProds[p] = true; });

  var rowsToDelete = [];
  var autoAvulsosToAdd = [];

  Object.keys(estadoMap).forEach(function(did) {
    if (!validIds[did]) {
      var e = estadoMap[did];
      if (lineKeyToConf[did] && e.status === 'Não Encontrado') {
        // Produto chegou sem etiqueta: mantém entrada ativa e vira avulso
        validIds[did] = true;
        if (!pendentesSet[e.produto]) {
          pendentes.push({ produto: e.produto, qtdNecessaria: e.qtd });
          pendentesSet[e.produto] = true;
        }
        if (!existingAvulsoProds[e.produto]) {
          autoAvulsosToAdd.push([e.produto, e.qtd]);
          existingAvulsoProds[e.produto] = true;
        }
      } else {
        rowsToDelete.push(e.row);
        delete estadoMap[did];
      }
    }
  });

  // Grava avulsos automáticos em Etiquetas_Avulsos
  if (autoAvulsosToAdd.length) {
    if (!avulsoSh) {
      avulsoSh = ss.insertSheet('Etiquetas_Avulsos');
      avulsoSh.getRange(1, 1, 1, 2).setValues([['Produto', 'Qtd']]);
      avulsoSh.getRange(1, 1, 1, 2).setFontWeight('bold');
    }
    avulsoSh.getRange(avulsoSh.getLastRow() + 1, 1, autoAvulsosToAdd.length, 2).setValues(autoAvulsosToAdd);
  }
