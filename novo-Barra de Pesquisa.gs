...
function getEtiquetasDoDia() {
  ...
  Object.keys(msgProdutosMap).forEach(function(msgId) {
    var pool = msgProdutosMap[msgId].slice(); // cópia mutável; candidatos vão sendo removidos
    var confRows = confMap[msgId];
    if (!confRows || !confRows.length) return;

    // Separa linhas normais de linhas de Kit (col D começa ou contém "Kit")
    var singleRows = confRows.filter(function(c) { return !/\bKit\b/i.test(c.produtoConf || ''); });
    var kitRows    = confRows.filter(function(c) { return  /\bKit\b/i.test(c.produtoConf || ''); });

    // 1ª passagem: linhas normais — score-based; remove o produto escolhido do pool
    singleRows.forEach(function(conf) {
      ...
      if (candidates.length === 1) {
        ...
        // Em caso de empate pega o primeiro — para linhas normais costuma haver um vencedor claro
    ...
    // 2ª passagem: linhas de Kit — expande todos os candidatos restantes no pool
    kitRows.forEach(function(conf) {
      if (!conf.marketplace) return;
      var candidates = pool.filter(function(p) { return produtoFaltaNoPedido(conf.status, p); });
      if (!candidates.length) return;
