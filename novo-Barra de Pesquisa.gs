...
function getEtiquetasDoDia() {
  ...
  Object.keys(msgProdutosMap).forEach(function(msgId) {
    ...
    confRows.forEach(function(conf) {
      ...
      // Verifica se o status lista múltiplos produtos via ";"
      var faltaRaw    = conf.status.replace(/^FALTA\s*-\s*/i, '').trim();
      var numItensStatus = faltaRaw.split(/\s*;\s*/).filter(Boolean).length;

      if (candidates.length > 1 && numItensStatus > 1) {
        // Status com vários produtos separados por ";" → uma entrada por produto encontrado
        ...
        // Caso normal: desambigua por pontuação de overlap com col D da Conferencia
