...
function getEtiquetasDoDia() {
  ...
  // Mapeia msgId → [produtos da Lista col D, em ordem de linha]
  var msgIdToListaProds = {};
  listaDEF.forEach(function(r) {
    var produtoD = String(r[0] || '').trim(); // col D
    var idsRaw   = String(r[2] || '').trim(); // col F
  ...
  // Constrói newPedidos: itera confMap (garante uma entrada por lineKey) e emparelha
  // com Lista col D pelo índice — linha 0 da Lista → linha 0 da Conferência, etc.
  var newPedidos = [];

  Object.keys(confMap).forEach(function(msgId) {
    var confRows    = confMap[msgId];
    var listaProds  = msgIdToListaProds[msgId] || [];

    confRows.forEach(function(conf, idx) {
      if (!conf.marketplace || !conf.lineKey) return;
      // Produto: Lista col D pelo índice correspondente;
      // fallback: último produto da Lista (combo 1→N); último fallback: Conferência col D
      var produtoD = listaProds[idx] || listaProds[listaProds.length - 1] || conf.produtoConf;
