...
function getEtiquetasDoDia() {
  ...
  Object.keys(msgProdutosMap).forEach(function(msgId) {
    ...
    confRows.forEach(function(conf) {
      ...
      // Quando há múltiplos candidatos (ex: Kit → 3 perfumes individuais), cria uma entrada por candidato.
      // Fix: condição original tinha "numItensStatus > 1" mas falha para "FALTA" puro com múltiplos produtos.
