  ...
  Object.keys(confMap).forEach(function(msgId) {
    ...
    confMap[msgId].forEach(function(conf) {
      ...
      subItens.forEach(function(subItem, idx) {
        var initialStatus;
        if (isTenho) {
          initialStatus = 'Encontrado';                        // tudo encontrado
        } else if (faltaIdx >= 0) {
          initialStatus = (idx === faltaIdx) ? '' : 'Encontrado'; // só o item FALTA fica em branco
        } else {
          initialStatus = '';                                  // FALTA genérico: todos em branco
        }
        newPedidos.push({
