...
function getEtiquetasDoDia() {
  ...
  // Mapeamento msgId → [produtos pendentes]: Lista col D + col F
  ...
  // Pedidos novos: cruza cada linha da Conferencia com produtos pendentes
  var newPedidos = [];
  Object.keys(msgProdutosMap).forEach(function(msgId) {
    ...
    confRows.forEach(function(conf) {
      ...
      // Filtra candidatos pelo status FALTA com keyword matching
      var candidates = produtos.filter(function(p) {
        return produtoFaltaNoPedido(conf.status, p);
      });
      if (!candidates.length) return;

      // Desambigua usando o nome do produto na Conferencia (col D) quando há mais de um candidato
      var matched = candidates[0];
      if (candidates.length > 1 && conf.produtoConf) {
        var confNome = conf.produtoConf.toLowerCase();
        var byName = candidates.filter(function(p) {
          return p.toLowerCase().split(/\s+/).filter(function(w) { return w.length > 2; })
                  .some(function(kw) { return confNome.indexOf(kw) !== -1; });
        });
        if (byName.length > 0) matched = byName[0];
      }
