  ...
  Object.keys(msgProdutosMap).forEach(function(msgId) {
    ...
    confRows.forEach(function(conf) {
      ...
      if (candidates.length > 1) {
        // Pontua cada candidato: quantas keywords do col D (nome do marketplace) aparecem no nome do candidato (col A da Lista).
        // Keywords vêm do conf.produtoConf para evitar problema de abreviações/pontuação nos nomes da Lista.
        var confNomeLower = (conf.produtoConf || '').toLowerCase().replace(/[^a-z0-9çáéíóúãõâêôàèìòù]/g, ' ');
        var confKws = confNomeLower.split(/\s+/).filter(function(w) { return w.length > 2; });
        var scores = candidates.map(function(p) {
          if (!confKws.length) return 0;
          var pLower = p.toLowerCase();
          return confKws.filter(function(kw) { return pLower.indexOf(kw) !== -1; }).length;
        });
        var maxScore = Math.max.apply(null, scores);
        var topCandidates = maxScore > 0
          ? candidates.filter(function(_, i) { return scores[i] === maxScore; })
          : candidates;
        if (topCandidates.length === 1) {
          // Único candidato com maior score → cada linha da Conferência tem seu produto
          newPedidos.push({
            discordId:   conf.lineKey,
            produto:     topCandidates[0],
            idPedido:    conf.idPedido,
            marketplace: conf.marketplace,
            qtd:         conf.qtd
          });
        } else {
          // Empate ou nenhum match → Kit: 1 linha da Conferência representa N produtos individuais
