...
function salvarECopiarMarketplace(currentMktJson) {
  ...
  // Conjunto de produtos da Lista que ainda estão ativos na Conferência (status ≠ SIM)
  var prodAtivo = {};
  try {
    var extSS = SpreadsheetApp.openById(LISTA_SHEET_ID);
    ...
    // Varre Conferência: produtos com Confirmado ≠ SIM (col L) são marcados como ativos
    var confSheet = extSS.getSheetByName('Conferencia');
    if (confSheet && confSheet.getLastRow() >= 2) {
      ...
      confSheet.getRange(2, 1, confSheet.getLastRow() - 1, numCols).getValues().forEach(function(r) {
        ...
        if (confirmado === 'SIM') return; // pedido confirmado — pode sumir do diff
  ...
  salvarHistoricoMarketplace(currentMktJson);
