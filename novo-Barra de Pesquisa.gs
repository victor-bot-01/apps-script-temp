...
function getEtiquetasDoDia() {
  ...
  // Mapeia msgId → [produtos da Lista col D] e registra quais msgIds existem na Lista
  var msgIdToListaProds = {};
  var msgIdsNaLista = {};
  listaDEF.forEach(function(r) {
    var produtoD = String(r[0] || '').trim();
    var idsRaw   = String(r[2] || '').trim();
    if (!idsRaw) return;
    idsRaw.split(/\s+/).forEach(function(msgId) {
      if (!msgId) return;
      msgIdsNaLista[msgId] = true;
      if (!produtoD) return;
      if (!msgIdToListaProds[msgId]) msgIdToListaProds[msgId] = [];
      msgIdToListaProds[msgId].push(produtoD);
    });
  });

  // Atribui cada linha da Lista (col D) à melhor linha da Conferência (col D) por keywords.
  // Isso resolve combos: "Poseidon 100ml" e "Spray 10ml" ambos apontam para a mesma
  // linha "Kit Poseidon + Spray Portátil" → geram duas entradas em Etiquetas_Estado.
  function scoreMatchProd(listaNome, confNome) {
    var norm = function(s) {
    ...
    var kL = norm(listaNome), kC = norm(confNome), n = 0;
    kL.forEach(function(w) { if (kC.indexOf(w) !== -1) n++; });
    return n;
  }
  ...
  Object.keys(confMap).forEach(function(msgId) {
    ...
    var listaProds = (msgIdToListaProds[msgId] || []).slice();

    // Caso simples: apenas uma linha na Conferência → todas as linhas da Lista vão para ela
    if (confRows.length === 1) {
      ...
      if (!listaProds.length) {
        newPedidos.push({ discordId: conf.lineKey, produto: conf.produtoConf,
          idPedido: conf.idPedido, marketplace: conf.marketplace, qtd: conf.qtd, initialStatus: initialStatus });
      } else {
        listaProds.forEach(function(prod, idx) {
          newPedidos.push({ discordId: conf.lineKey + (idx > 0 ? '_' + idx : ''), produto: prod,
            idPedido: conf.idPedido, marketplace: conf.marketplace, qtd: conf.qtd, initialStatus: initialStatus });
        });
      }
      return;
    ...
    // Múltiplas linhas na Conferência: atribui cada linha da Lista à linha de Conferência
    // com maior score de keywords (ex.: "Poseidon" → "Kit Poseidon + Spray", "Eros" → "Eros 100ml")
    var assignments = confRows.map(function() { return []; }); // assignments[i] = listaProds para confRows[i]
    listaProds.forEach(function(prod) {
      var scores  = confRows.map(function(c) { return scoreMatchProd(prod, c.produtoConf); });
      var maxScore = Math.max.apply(null, scores);
      assignments[scores.indexOf(maxScore)].push(prod);
    });

    confRows.forEach(function(conf, ci) {
      if (!conf.marketplace || !conf.lineKey) return;
      var initialStatus = String(conf.status || '').trim().toUpperCase() === 'TENHO' ? 'Encontrado' : '';
      var prods = assignments[ci];
      if (!prods.length) {
        // Nenhuma linha da Lista atribuída a esta linha → usa Conferência col D como fallback
        newPedidos.push({ discordId: conf.lineKey, produto: conf.produtoConf,
          idPedido: conf.idPedido, marketplace: conf.marketplace, qtd: conf.qtd, initialStatus: initialStatus });
      } else {
