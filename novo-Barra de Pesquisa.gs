...
function getEtiquetasDoDia() {
  ...
  // Mapeamento discordId → produto: Lista col D (produto) + col F (IDs separados por espaço)
  ...
  listaDF.forEach(function(r) {
    ...
    idsRaw.split(/\s+/).forEach(function(id) { if (id) idProdutoMap[id] = produto; });
  });

  // Conferencia: discordId → { idPedido, marketplace, qtd, status }
  ...
  // Pedidos novos: somente FALTA para produtos pendentes
  var newPedidos = [];
...
// Limpa todo o conteúdo de Etiquetas_Estado (mantém cabeçalho)
