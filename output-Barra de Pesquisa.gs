...
function getEtiquetasDoDia() {
  ...
  if (pedidosSheet && pedidosSheet.getLastRow() >= 2) {
    ...
    pedidosData.forEach(function(r) {
      if (r[0]) pedidosSet[String(r[0])] = true;
    });
  }

  // Parts map — key: produto → array of { parte, qtdNaParte }
  // Supports the same product appearing in multiple rows (different parts)
  var partesMap = {};
  var partesSheet = ss.getSheetByName('Etiquetas_Partes');
  if (partesSheet && partesSheet.getLastRow() >= 2) {
    var partesData = partesSheet.getRange(2, 1, partesSheet.getLastRow() - 1, 3).getValues();
    partesData.forEach(function(r) {
      if (!r[0]) return;
      var key = String(r[0]);
      if (!partesMap[key]) partesMap[key] = [];
      partesMap[key].push({ parte: Number(r[1]) || 0, qtdNaParte: Number(r[2]) || 0 });
    });
  }
  ...
  pendentes.forEach(function(item) {
    var partes = partesMap[item.produto];

    if (!partes || !partes.length) {
      // No parts — single entry with parte 0
      var chave  = item.produto + '|0';
      var estado = estadoMap[chave] || { qtdEncontrada: 0, observacao: '' };
      result.push({
        produto:       item.produto,
        qtdNecessaria: item.qtdNecessaria,
        qtdEncontrada: Math.min(estado.qtdEncontrada, item.qtdNecessaria),
        encontrado:    estado.qtdEncontrada >= item.qtdNecessaria,
        observacao:    estado.observacao,
        locais:        locMap[item.produto] || [],
        pedidoMais:    !!pedidosSet[item.produto],
        parte:         0
      });
      return;
    }

    // One result entry per defined part
    var qtdTotalNasPares = partes.reduce(function(s, p) { return s + p.qtdNaParte; }, 0);
    partes.forEach(function(pi) {
      var chave  = item.produto + '|' + pi.parte;
      var estado = estadoMap[chave] || { qtdEncontrada: 0, observacao: '' };
      result.push({
        produto:       item.produto,
        qtdNecessaria: pi.qtdNaParte,
        qtdEncontrada: Math.min(estado.qtdEncontrada, pi.qtdNaParte),
        encontrado:    estado.qtdEncontrada >= pi.qtdNaParte,
        observacao:    estado.observacao,
        locais:        locMap[item.produto] || [],
        pedidoMais:    !!pedidosSet[item.produto],
        parte:         pi.parte
    ...
    // Extra "Novos" entry when total quantity grew after the last division
    if (item.qtdNecessaria > qtdTotalNasPares) {
      var qtdExtra = item.qtdNecessaria - qtdTotalNasPares;
      var chaveEx  = item.produto + '|0';
      var estadoEx = estadoMap[chaveEx] || { qtdEncontrada: 0, observacao: '' };
      result.push({
        produto:       item.produto,
        qtdNecessaria: qtdExtra,
        qtdEncontrada: Math.min(estadoEx.qtdEncontrada, qtdExtra),
        encontrado:    estadoEx.qtdEncontrada >= qtdExtra,
        observacao:    estadoEx.observacao,
        locais:        locMap[item.produto] || [],
        pedidoMais:    !!pedidosSet[item.produto],
        parte:         0
      });
...
// key is now "produto|parte"
function salvarEstadoEtiqueta(produto, parte, qtdEncontrada, observacao) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sh = ss.getSheetByName('Etiquetas_Estado');
  if (!sh) {
    sh = ss.insertSheet('Etiquetas_Estado');
    sh.getRange(1, 1, 1, 3).setValues([['Chave', 'Qtd Encontrada', 'Observação']]);
    sh.getRange(1, 1, 1, 3).setFontWeight('bold');
  }

  var chave = produto + '|' + (parte || 0);
  var lastRow = sh.getLastRow();
  var existingRow = -1;
  if (lastRow >= 2) {
  ...
  if (existingRow > 0) {
    ...
    sh.appendRow([chave, qtdEncontrada, observacao]);
  }
  return { sucesso: true };
}
...
function limparPartesEtiqueta() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sh = ss.getSheetByName('Etiquetas_Partes');
  if (!sh || sh.getLastRow() < 2) return { sucesso: true };
  sh.getRange(2, 1, sh.getLastRow() - 1, 3).clearContent();
  return { sucesso: true };
}

// Now receives qtds array (parallel to produtos) to store qty at division time
function criarParteEtiqueta(produtos, qtds) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sh = ss.getSheetByName('Etiquetas_Partes');
  if (!sh) {
    sh = ss.insertSheet('Etiquetas_Partes');
    sh.getRange(1, 1, 1, 3).setValues([['Produto', 'Parte', 'Qtd_Na_Parte']]);
    sh.getRange(1, 1, 1, 3).setFontWeight('bold');
  }
  ...
  if (sh.getLastRow() >= 2) {
    ...
    existing.forEach(function(r) {
      var n = Number(r[1]);
      if (!isNaN(n) && n > maxParte) maxParte = n;
    });
  }
  var numParte = maxParte + 1;

  var rows = produtos.map(function(p, i) {
    return [p, numParte, (qtds && qtds[i]) ? Number(qtds[i]) : 1];
  });
  if (rows.length) {
    sh.getRange(sh.getLastRow() + 1, 1, rows.length, 3).setValues(rows);
  }

  return { sucesso: true, numParte: numParte };
}
...
function pedirMaisEtiqueta(produto, qtdNecessaria, local) {
  ...
  if (lastRow >= 2) {
    var data = sh.getRange(2, 1, lastRow - 1, 1).getValues();
    for (var i = 0; i < data.length; i++) {
      if (String(data[i][0]) === String(produto)) {
...
function produtoFaltaNoPedido(status, nomeProduto) {
  ...
  var faltaItem = status.replace(/^FALTA\s*-\s*/i, '').trim().toLowerCase();
  var keywords  = faltaItem.split(/\s+/).filter(function(w) { return w.length > 1; });
  var nome      = nomeProduto.toLowerCase();
  return keywords.every(function(kw) { return nome.indexOf(kw) !== -1; });
}

function getMarketplacesParaProdutos(produtosVisiveis) {
  if (!produtosVisiveis || !produtosVisiveis.length) return {};

  var extSS      = SpreadsheetApp.openById(LISTA_SHEET_ID);
  var listaSheet = extSS.getSheetByName('Lista');
  if (!listaSheet || listaSheet.getLastRow() < 2) return {};

  // Lista col D (4) = produto, col E (5) = qty, col F (6) = space-separated DiscordMessageIds
  var listaData = listaSheet.getRange(2, 4, listaSheet.getLastRow() - 1, 3).getValues();

  // Build discordId → produto name (from Lista col D+F)
  var idProdutoMap = {};
  listaData.forEach(function(r) {
    var produto = String(r[0] || '').trim();
    var idsRaw  = String(r[2] || '').trim();
    if (!produto || !idsRaw) return;
    idsRaw.split(/\s+/).forEach(function(id) {
      if (id) idProdutoMap[id] = produto;
    });
  });

  // Read Conferência sheet
  var confSheet = extSS.getSheetByName('Conferência');
  if (!confSheet || confSheet.getLastRow() < 2) return {};

  // Build discordId → { marketplace, status }
  // Columns: A=Pedido, B=Marketplace, C=Cliente, D=Produto, E=Qtd, F=Status, G=ConferidoEm,
  //          H=ConferidoPor, I=DiscordMessageId (index 8)
  var confMap  = {};
  var confData = confSheet.getRange(2, 1, confSheet.getLastRow() - 1, 9).getValues();
  confData.forEach(function(r) {
    var marketplace = String(r[1] || '').trim(); // col B
    var status      = String(r[5] || '').trim(); // col F
    var discordId   = String(r[8] || '').trim(); // col I
    if (!discordId) return;
    confMap[discordId] = { marketplace: marketplace, status: status };
  });

  // Filter to visible products set for fast lookup
  var produtosSet = {};
  produtosVisiveis.forEach(function(p) { produtosSet[p] = true; });

  // Aggregate: marketplace → { produto → count }
  var result = {};
  Object.keys(idProdutoMap).forEach(function(discordId) {
    var produto = idProdutoMap[discordId];
    if (!produtosSet[produto]) return;

    var conf = confMap[discordId];
    if (!conf || !conf.marketplace) return;
    if (!produtoFaltaNoPedido(conf.status, produto)) return;

    var mkt = conf.marketplace;
    if (!result[mkt]) result[mkt] = {};
    result[mkt][produto] = (result[mkt][produto] || 0) + 1;
  });
